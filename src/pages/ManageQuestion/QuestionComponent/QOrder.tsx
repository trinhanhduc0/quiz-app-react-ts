...
export default function OrderQuestionEditor({ options, onChange }: OrderQuestionEditorProps) {
  ...
  const updateOptionText = (index: number, key: keyof OrderItem, value: string | number) => {
    const newOptions = [...options];
  ...
  const addOption = () => {
    onChange([...options, { id: '', text: "", order: options.length + 1 }]);
  };
    ...
    <div className="space-y-4">
      {options.map((option, index) => (
        <div key={index} className="border rounded-md p-4 space-y-2">
          <textarea
            value={option.text}
            onChange={(e) => updateOptionText(index, "text", e.target.value)}
